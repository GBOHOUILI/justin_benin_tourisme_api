...
class CatSiteController extends Controller
{
    ...
    {
        $validated = $request->validate(['libelle' => 'required|string|max:150|unique:cat_sites']);
    ...
    {
        $validated = $request->validate(['libelle' => 'required|string|max:150|unique:cat_sites,libelle,' . $catSite->id]);
