...
- [x] **Confirmé (2026-09-17)** : `AdminEvenements.jsx`/`AdminCategories.jsx`/`AdminPrix.jsx`/`AdminUsers.jsx` audités individuellement (grep hex v1 + `var(--x)` non définies, comme pour les 4 fichiers du 2026-09-17) — aucun résidu, hypothèse d'héritage via les classes partagées confirmée. Vérifié en réel (Playwright, les 4 pages) : 0 erreur console. Les puces de filtre de `AdminAvis.jsx` restent en forme de pilule (pas alignées sur le nouveau style rectangulaire des filtres publics) — cosmétique, non traité.
