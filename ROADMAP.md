...
- [ ] **Bug de logique non corrigé (hors périmètre CSS/layout, déjà noté)** : `AdminAdmins.jsx` affiche le statut brut (`1`) au lieu de "Actif" — comparaison `a.status === 'actif'` contre une valeur booléenne/entière renvoyée par l'API
- [ ] `AdminLayout.jsx`/`AdminLogin.jsx` : pas retouchés cette passe, déjà cohérents visuellement dans tous les screenshots pris (sidebar, header, page de connexion)
